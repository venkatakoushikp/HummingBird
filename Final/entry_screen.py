...
humm_bird.title("Humming Bird - Deep Packet Inspection tool")
...
intro_text=tk.Label(humm_bird,text="Humming Bird - Deep packet Inspection Tool",font=('Times new roman',25))
...
def on_grab():
    ...
    text1=tk.Label(grabber,text="Mention Host Name or IP:")
        ...
        try:
            ...
            def scp_file(source_path, destination_path, hostname, username, password):
                try:
                    ...
                    scp.close()
                    client.close()
                    messagebox.showinfo("","File grabbed succesfully")
    ...
    text11=tk.Label(grabber,text="Mention Host Name or IP:")
...
def on_att():
    ...
    if dropdown.get()=="OSPF":
        ...
        new_win=tk.Toplevel()
        new_win.title("OSPF - Provide IP Address of OSPF neighbours")
        new_win.geometry('500x160')
        new_win.resizable(0,0)
        ip1_1_t=tk.Label(new_win,text="Provide IP Address of the 1st OSPF neighbour",font=("times new roman",16))
        ip1_1_t.place(x=10,y=10)
        ip1_in=tk.Entry(new_win,width=10)
        ip1_in.place(x=350,y=10)
        ip1_2_t=tk.Label(new_win,text="Provide IP Address of the 2nd OSPF neighbour",font=("times new roman",16))
        ...
        new_win.geometry('500x160')
        new_win.resizable(0,0)
        ip1_1_t=tk.Label(new_win,text="Provide MAC Address of the 1st interface",font=("times new roman",16))
        ip1_1_t.place(x=10,y=10)
        ip1_in=tk.Entry(new_win,width=10)
        ip1_in.place(x=350,y=10)
        ip1_2_t=tk.Label(new_win,text="Provide MAC Address of the 2nd Interface",font=("times new roman",16))
        ip1_2_t.place(x=1110,y=50)
        ip2_in=tk.Entry(new_win,width=10)
        ip2_in.place(x=1350,y=50)
        ...
        def on_next():
            ...
            b=ip2_in.get()
            if ((a==b) or ((b=="") and (a==""))):
                messagebox.showerror("","Mention Valid MAC address")
...
def on_grab():
    ...
    def on_gen():
        ...
        try:
            ...
            Exception
            mb.showerror("","Hostname/IP not Reachable \n\n\n Please confirm that the hostname or IP address provided is reachable.")
    ...
    if vars["filename_1"]=="":
        ...
        if vars['Pathlength']==1:
            ...
            if vars["protocol"]=="OSPF":
                ...
                from Demo_ import humm_bird as k
                k.mainloop()
            ...
            if vars["protocol"]=="OSPF":
                ...
                humm_bird.destroy()
                import ospfday2
                from demo_2pcap import humm_bird as k
                k.mainloop()
...
def on_help():
    ...
    help_w.geometry('400x500')
    label1=tk.Label(help_w,text="HELP PAGE",font=('Times New Roman',15))
    label1.pack()
    label2=tk.Label(help_w,text='''OSPF:

    1.Select The protocol
    2.Click on Mention Attributes
    3.Mention the IP addresses of the OSPF neighbors
    4.Either upload the PCAP from local device or from switch
    5.Click on Proceed
...
LACP:
    ...
    1.Select The protocol
    2.Click on Mention Attributes
    3.Mention the MAC addresses of the Interfaces
    4.Either upload the PCAP from local device or from switch
    5.Click on Proceed
    ''',justify='left',font=('Times New Roman',15))
    label2.place(x=2,y=20)
    help_w.mainloop()
helps=tk.Button(humm_bird,text="Help",command=on_help)
helps.place(x=832,y=350)
