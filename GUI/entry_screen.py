...
def on_att():
    if dropdown.get()=="OSPF":
        ...
        messagebox.showerror("","Currently, the tool only supports OSPF")
...
def on_grab():
    ...
    def on_gen2():
        try:
            ...
            selected_fil2.configure(text="Selected File :{}".format(vars["filename_2"]))
        except:
            Exception
            mb.showerror("","Please Recheck the inputs")
...
up_p=tk.Button(humm_bird,text="Upload PCAP 1",width=10,command=on_open)
up_p.place(x=400,y=200)
up_p2=tk.Button(humm_bird,text="Upload PCAP 2",width=10,command=on_open2)
